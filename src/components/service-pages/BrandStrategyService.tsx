...
export default function BrandStrategyService() {
  ...
  useEffect(() => {
    document.title = 'Brand Strategy — NSS';
    ...
    script.id = 'brand-strategy-faq-schema';
    script.textContent = JSON.stringify(schema);
    document.head.appendChild(script);

    return () => {
      document.title = 'NSS — Software that ships';
      document.getElementById('brand-strategy-faq-schema')?.remove();
    };
  }, []);
  ...
  return (
    <article className="min-h-screen bg-white">
      ...
      <header className="webdev-hero relative border-b border-white/10 px-6">
        <ServiceHeroBackground image={HERO_IMAGES.brandStrategy} />
