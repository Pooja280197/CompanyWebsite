...
const HERO_TAGLINE = `Software shaped to your business — not the other way around.`;

const HERO_LETTER_INTERVAL = 72;
const HERO_TITLE_LINES = [
  {
    words: [{ text: 'CUSTOM' },{ text: 'SOFTWARE ' }],
    letterCount: 6,
    inline: true,
    startDelay: 0,
  },
...
export default function CustomSoftware() {
  ...
  useEffect(() => {
    document.title = 'Custom Software — NSS';
    ...
    script.id = 'custom-software-faq-schema';
    script.textContent = JSON.stringify(schema);
    document.head.appendChild(script);

    return () => {
      document.title = 'NSS — Software that ships';
      document.getElementById('custom-software-faq-schema')?.remove();
    };
  }, []);
  ...
  return (
    <article className="min-h-screen bg-white">
      ...
      <header className="webdev-hero relative border-b border-white/10 px-6">
        <ServiceHeroBackground image={HERO_IMAGES.customSoftware} />
