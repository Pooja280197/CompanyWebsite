...
const RELEASE_TAX_WORDS = [
  { text: 'The' },
  { text: 'release' },
  { text: 'tax' },
];

const MANUAL_PITFALLS = [
  'Engineer hours lost to every manual step',
  'Skipped tests under deadline pressure',
  'Weekend morale spent on deploys',
];

const PIPELINE_TAGS = ['CI/CD automation', 'Quality gates', 'Rollback paths'];

const HERO_TAGLINE = `Make releases boring.`;
const HERO_DESC = `If deployments need a Saturday, a war room, and a prayer, the pipeline is the product blocking your product. Our DevOps consulting services fix the pipeline.`;
...
const HERO_TITLE_LINES = [
  ...
  },
  {
    ...
    letterCount: 8,
    inline: false,
    startDelay: 6 * HERO_LETTER_INTERVAL,
  },
];
...
const BUILD_ITEMS = [
  {
    ...
    desc: 'Commit-to-production automation with Jenkins, quality gates, and rollback paths',
    icon: GitBranch,
  ...
  },
  {
    ...
    desc: 'Docker and Kubernetes for consistent environments from laptop to production',
    icon: Container,
  ...
  },
  {
    ...
    desc: 'Terraform-managed environments that rebuild identically, every time',
    icon: Layers,
  ...
  },
  {
    ...
    desc: 'Secrets management, dependency scanning, and compliance checks inside the pipeline (not after the breach)',
    icon: ShieldCheck,
  ...
  },
  {
    ...
    desc: 'Observability that pages the right person with the right context',
    icon: BarChart3,
  ...
  },
  {
    ...
    desc: 'Scalable AWS, Azure, and GCP environments engineered for reliability, cost control, and high availability',
    icon: Cloud,
  ...
  },
  {
    ...
    desc: 'Blue-green, canary, and zero-downtime deployment strategies that minimize risk while keeping services online',
    icon: Workflow,
...
const PROOF_STATS = [
  ...
  { value: '2', label: 'Embedded engineers' },
  { value: '100%', label: 'Pipeline inherited' },
];
...
const GROWTH_INTRO =
  'Jenkins for commit-to-production automation. Terraform for environments that rebuild identically. Docker and Kubernetes from laptop to production. Security and compliance checks inside the pipeline — not after the breach.';

const PROOF_INTRO =
  'A manufacturing company faced deployment delays and security risk in their CI/CD during a critical project phase. Two of our embedded DevOps specialists automated the deployment process, hardened pipeline security, and aligned it with industry compliance: deployment times down 25%, seamless project completion, and a pipeline the next project inherited instead of rebuilding.';

const FAQS: { q: string; a: ReactNode }[] = [
  {
    q: 'Project or embedded engineers — which fits us?',
    a: (
      <>
  ...
  {
    q: 'How long does CI/CD implementation take?',
    a: 'A working pipeline for one product: 3–6 weeks. Organization-wide standards: phased over a quarter. The audit scopes it precisely.',
  },
  {
    q: 'We deploy manually and it mostly works. Why change?',
    a: '\'Mostly\' is the cost. Count the engineer hours per release, the delayed hotfixes, the features held back to batch a deploy — that number funds the automation several times over.',
  },
  {
    q: 'Kubernetes — do we actually need it?',
    a: 'Not always, and we\'ll say so. Small stable workloads often run cheaper without it. Orchestration earns its complexity at scale, with microservices, or with variable load.',
  },
  {
    q: 'Can you fix a pipeline another team built?',
    a: 'Yes — the audit maps what exists, what\'s fragile, and what\'s dangerous, then we repair incrementally. Rip-and-replace is the last resort.',
  },
];
...
export default function DevopsService() {
  ...
  useEffect(() => {
    document.title = 'DevOps Services — NSS';
    const meta = document.querySelector('meta[name="description"]');
    if (meta) {
      meta.setAttribute(
        'content',
        'DevOps services — CI/CD pipelines, infrastructure as code, pipeline security, and observability that cut the release tax.',
      );
    }
    window.scrollTo(0, 0);
      ...
      mainEntity: FAQS.map((faq) => ({
        ...
        acceptedAnswer: {
          ...
          text: typeof faq.a === 'string'
            ? faq.a
            : 'Both work: we build your pipeline foundation as a fixed-scope project, or our specialists embed in your team. The manufacturing case above was two embedded engineers.',
        },
      })),
    };
    ...
    script.id = 'devops-faq-schema';
    script.textContent = JSON.stringify(schema);
    document.head.appendChild(script);

    return () => {
      document.title = 'NSS — Software that ships';
      document.getElementById('devops-faq-schema')?.remove();
    };
  }, []);
  ...
  return (
    <article className="min-h-screen bg-white">
      ...
      <header className="webdev-hero relative border-b border-white/10 px-6">
        <ServiceHeroBackground image={HERO_IMAGES.devops} />
      ...
      {/* The release tax */}
      <section className="py-20 px-6 overflow-hidden" aria-labelledby="release-tax-heading">
        <div className="max-w-[1200px] mx-auto w-full">
          <div className="text-center mb-14 sr w-full max-w-[920px] mx-auto px-2">
            <ScrollTextReveal
              id="release-tax-heading"
              tag="h2"
              align="center"
              className="w-full"
              wordGap="0.2em"
              ...
              words={RELEASE_TAX_WORDS}
            />

            <p className="sr sr-d2 text-[#555] text-base sm:text-lg leading-[1.85] mt-8 max-w-[48rem] mx-auto text-center">
              Every manual deployment step is a tax paid in engineer hours and weekend morale — and every
              skipped test under deadline pressure is a loan against production. Automation isn&apos;t a
              luxury for big teams; it&apos;s how small teams ship like big ones.
            </p>
          </div>
            ...
            <article className="webdev-compare-card webdev-compare-card--custom sr-from-left">
              <div className="webdev-compare-card__inner">
                ...
                <p className="webdev-compare-card__desc">
                  Commit-to-production pipelines with quality gates and rollback paths — so small teams
                  ship with the reliability big teams take for granted.
                </p>
      ...
      {/* What we implement — pinned split + scroll-driven card list */}
      <section ref={buildSectionRef} className="webdev-build-pin px-6 bg-white" aria-labelledby="what-implement">
              ...
              <div className="webdev-build-layout grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 xl:gap-16 items-stretch">
                <div className="webdev-build-left sr webdev-build-left--has-tags">
                  ...
                  <p className="webdev-build-left__desc text-[#666] text-base sm:text-lg leading-relaxed max-w-[28rem]">
                    Pipelines, containers, infrastructure as code, and observability — built to cut the
                    release tax, not add another tool bill.
                  </p>
                ...
                <div className="webdev-build-scroll-wrap sr sr-d2">
                  <div
                    ...
                    role="region"
                    aria-label="What we implement services"
                  >
        ...
        <div className="max-w-[1200px] mx-auto w-full webdev-panel-stack">
          <article className="webdev-panel webdev-panel--img-left sr-from-center">
            <div className="webdev-panel__media">
              <img
                src={GROWTH_IMAGE}
                alt="DevOps pipelines and infrastructure automation"
                loading="lazy"
              />
            </div>
